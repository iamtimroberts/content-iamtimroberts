...
# 3. Docker or podman is installed (if neither are installed, you can skip image download using the `-sd` option)
# 4. Docker or podman python is installed (install it by running "pip install <docker|podman>" 
#    or "pip3 install <docker|podman>" or use the `-sd` option)
# 5. If using podman, the podman socket needs to be exposed. See podman_socket argument and below article for more information
...
def get_docker_images_with_tag(pack_names: dict, id_set_json: dict) -> set:
    ...
    print("Starting to collect images")
    ...
    for pack_d_name, pack_name in pack_names.items():
        ...
        integrations = (
            content_items["integrations"] if "integrations" in content_items else []
        )
        scripts = content_items["scripts"] if "scripts" in content_items else []
        if integrations:
            print(f"\t{pack_d_name} images found for integrations:")
        ...
        if scripts:
            print(f"\t{pack_d_name} images found for scripts:")
...
def download_and_save_packs(
    pack_names: dict, id_set_json: dict, output_path: str, verify_ssl: bool
) -> None:
    ...
    try:
        for pack_d_name, pack_name in pack_names.items():
            if pack_name not in id_set_packs:
                print(
                    f"\tCouldn't find {pack_d_name} in id_set.json. Skipping pack download."
                )
                continue
...
def download_and_save_docker_images(
    docker_images: set, output_path: str, podman_socket: str, use_podman: bool = False
) -> None:
    """Downloads and saves the docker images into docker.zip in output_path"""
    if use_podman:
        import podman  # import podman only when required

        print("Starting to download podman images for given packs")
        cli = podman.PodmanClient(base_url=f"unix://{podman_socket}")
    else:
    ...
    try:
        for image in docker_images:
            ...
            image_file_name = os.path.join(
                temp_dir.name, os.path.basename(f"{image_pair[0]}_{image_pair[1]}.tar")
            )
...
def options_handler():
    ...
    parser = argparse.ArgumentParser(
        description="Downloads XSOAR packs as zip and their latest images as tar."
    )
    parser.add_argument(
        "-p",
        "--packs",
    ...
    parser.add_argument(
        "--use_podman",
        help="Use podman sdk instead of docker sdk for image downloading.",
        required=False,
        action="store_true",
    )
    parser.add_argument(
        "--podman_socket",
        help="Path to podman socket if other than /var/run/podman/podman.sock",
        required=False,
...
def main():
    ...
    if not options.skip_packs and pack_names:
        download_and_save_packs(
            pack_names, id_set_json, os.path.join(output_path, "packs"), verify_ssl
        )
    else:
    ...
    if pack_names:
        ...
        if not options.skip_docker:
            download_and_save_docker_images(
                docker_images, os.path.join(output_path, "docker"), options.podman_socket, options.use_podman
            )
        else:
